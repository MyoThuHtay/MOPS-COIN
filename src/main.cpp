...
#if defined(NDEBUG)
# error "Mopscoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x510fb8ae68a95cbe7d47b4340fa964012ee47a0522f75d80a383d53b42cecd67");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Mopscoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Mopscoin Signed Message:\n";
...
{
    // Mopscoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Mopscoin: 420k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 2 * 24 * 60 * 60; // Mopscoin: 2 days
static const int64 nTargetSpacing = 5; // Mopscoin: 5 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Mopscoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Mopscoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x533b5effc58d209e6015911f375226bcbdf404740e82311fde4cc0a0c5771783");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "June 08 2023 - MOPS COIN was Born";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04263fbe701be5de1f920549c611158fe2a4621b4e031da8d864b3eb0507059c99f713578faddd536d9916071786651ee52a589b8e6a0b1b8a57e46d71dddf5c56") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1686233377;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085274912;

        if (fTestNet)
        {
            block.nTime    = 1686233322;
            block.nNonce   = 385742463;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x301f7bbe57a8ae2cbddca6b4886e034baa14cc501fbaa12ea29906d63060cd37"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc2, 0xb3, 0xdd }; // Mopscoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MopscoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MopscoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MopscoinMiner(CWallet *pwallet)
{
    printf("MopscoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("mopscoin-miner");
    ...
    try { loop {
        ...
        printf("Running MopscoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MopscoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MopscoinMiner, pwallet));
}
